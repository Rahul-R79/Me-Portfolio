...
export default function GithubGraph() {
        ...
        async function fetchData() {
            try {
                ...
                if (json.contributions) {
                    ...
                    // "Ghost Padding" Strategy:
                    // Pad data to the end of the current month so the library renders the month label.
                    // We will visually hide these extra days in renderBlock.
                    const lastEntry = contributions[contributions.length - 1];
                    const lastDate = new Date(lastEntry.date);
                    const today = new Date();
                    const endOfMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0);

                    // If our last data point is before end of month, pad it
                    if (lastDate < endOfMonth) {
                ...
                console.error("Failed to fetch Github Graph:", error);
            } finally {
                setLoading(false);
            }
        }
    ...
    if (loading) {
        return (
            <div className="w-full h-40 flex items-center justify-center border border-white/10 bg-white/5 rounded-2xl animate-pulse">
                <span className="text-gray-500 font-mono text-sm">Loading Graph...</span>
            </div>
        );
    }
    ...
    return (
        ...
        >
            ...
            <motion.div
                className="absolute inset-0 bg-[linear-gradient(45deg,transparent_25%,rgba(34,197,94,0.6)_50%,transparent_75%,transparent_100%)] bg-[length:250%_250%,100%_100%]"
            ...
            <div className="relative h-full w-full bg-[#161616] backdrop-blur-md rounded-xl p-6 flex flex-col items-center justify-center overflow-hidden">
                <div className="w-full flex justify-between items-center mb-4">
                    <h3 className="font-bold text-gray-200 flex items-center gap-2">
                        <span className="text-green-400">■</span> Contribution Graph
                    </h3>
                    <div className="relative">
                        <span className="text-xs text-gray-500 font-mono transition-opacity duration-300 group-hover:opacity-0">{total} contributions in the last year</span>
                ...
                {/* Scroll container for mobile safety */}
